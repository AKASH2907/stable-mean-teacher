...
class CapsNet(nn.Module):
    ...
    def forward(self, img, classification):
        ...
        if self.training:
            # print(type(caps), type(classification))
            # print(torch.eye(caps).is_cuda, classification.is_cuda)
            activations = torch.eye(caps).to('cuda')[classification.long()]
...
if __name__ == '__main__':
    ...
    # print(fstack.shape)
    out, ap, feat_shape = model(fstack, actor, 'unlabel')
