...
def val_model_interface(minibatch):
    ...
    # st_loc_pred_aux = erc_net(torch.sigmoid(st_loc_pred))
    # t_loc_pred_aux = ema_erc_net(torch.sigmoid(t_loc_pred))
    ...
    # loc_loss3 = criterion_loc_1(st_loc_pred_aux, label_mask)
    # loc_loss4 = criterion_loc_2(st_loc_pred_aux, label_mask)
...
def train_model_interface(args, label_minibatch, unlabel_minibatch, epoch, global_step, wt_ramp):
    ...
    # st_pred_labeled = erc_net(torch.flip(labeled_op_sig, [2]))
    # sup_loc_loss_3 = criterion_loc_1(torch.flip(st_pred_labeled, [2]), labeled_loc_data)
    # sup_loc_loss_4 = criterion_loc_2(torch.flip(st_pred_labeled, [2]), labeled_loc_data)
    #############################################################
...
if __name__ == '__main__':
    ...
    for e in tqdm(range(1, N_EPOCHS + 1), total=N_EPOCHS, desc="Epochs"):
        ...
        # if e%5==0 and e<30:
        #     torch.save(model.state_dict(), osp.join(model_save_dir, f'main_model_verify_ep_{e}.pth'))
        #     torch.save(erc_net.state_dict(), osp.join(model_save_dir, f'aux_model_verify_ep_{e}.pth'))
